        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ url('/penjualan/'.$penjualan->penjualan_id) }}" class="form-horizontal">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-10">
                        <select class="form-control" id="user_id" name="user_id" required value="{{ old('user_id', $penjualan->user_id) }}">
                            <option value="">- Pilih PIC -</option>
                            @foreach($user as $item)
                            <option value="{{ $item->user_id }}" {{ $item->user_id == $penjualan->user_id ? 'selected' : '' }}>{{ $item->username }}</option>
                            @endforeach
                        </select>
                        @error('username')
                ...
                <div id="detail">
                   {{-- looping detail penjualan --}}
                    @foreach($penjualan->detail_penjualan as $key => $detail)
                    <div id="row{{ $key + 1 }}">
                        <div class="form-group row">
